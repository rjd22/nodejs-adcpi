...
MCP3424 = (function() {
    ...
    MCP3424.prototype.getVoltage = function(chan) {
        // Somehow LSB does not work so I hardcoded it for now to 0.0005
    ...
    MCP3424.prototype._getLsb = function() {
        var resolution = this.res << 2;

        if (resolution == MCP342X_12_BIT) {
            return 0.0005;
        }

        if (resolution == MCP342X_14_BIT) {
            return 0.000125;
        }

        if (resolution == MCP342X_16_BIT) {
            return 0.00003125;
        }

        if (resolution == MCP342X_18_BIT) {
            return 0.0000078125;
        }
    };
